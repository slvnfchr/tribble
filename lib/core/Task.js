	...
	constructor(parameters) {
		...
		this.run = (configuration, cb) => {
			...
			if (_components.length > 0) {
				receiver = new core.Component((input, output) => {
					const file = input.read();
					output.send(file);
				});
				_components.forEach((component) => {
					component.root.mediatypes.input.forEach((mediatype) => {
						const reader = _getGraphComponent(path.resolve(__dirname, '../plugins/reader'), 10);
				...
				receiver = _getGraphComponent(path.resolve(__dirname, '../plugins/reader'), 10);
			}
